...
import kr.ac.konkuk.koogle.Adapter.RecommandAdapter
...
class ProfileActivity : AppCompatActivity() {
    var tag_debug_data:ArrayList<TagData> = ArrayList()
    var recommand_debug_data:ArrayList<ArrayList<String>> = ArrayList()
    lateinit var binding : ActivityProfileBinding
    lateinit var tagRecyclerView: RecyclerView
    lateinit var recommandRecyclerView: RecyclerView
    ...
    private fun initRecommandRecyclerView(){
        recommandRecyclerView = findViewById(R.id.recommendRecyclerView)
        recommandRecyclerView.layoutManager = LinearLayoutManager(this)
        // 구분선 넣기
        recommandRecyclerView.addItemDecoration(DividerItemDecoration(tagRecyclerView.context, 1))
        recommandAdapter = RecommandAdapter(this, recommand_debug_data)
        // 아이템 클릭 리스터 설정(미구현)
        recommandRecyclerView.adapter = recommandAdapter
    }
    ...
    private fun initData(){
        ...
        tag_debug_data.add(TagData("취미", arrayListOf("영화감상", "게임", "서핑",
                "여행", "독서", "술", "요리", "그림그리기")))
        tag_debug_data.add(TagData("전공", arrayListOf("컴퓨터", "컴퓨터공학")))
        tag_debug_data.add(TagData("언어", arrayListOf("한국어", "영어")))
        tag_debug_data.add(TagData("성격", arrayListOf("활동적인", "솔직한")))
        tag_debug_data.add(TagData("해외여행", arrayListOf("러시아", "태국", "중국",
        ...
        recommand_debug_data.add(arrayListOf("닉네임1", "ㅇㅇㅇ 교환 했었는데 친절하셨습니다 기분좋게" +
                "거래했네요! 어쩌고 저쩌고~"))
        recommand_debug_data.add(arrayListOf("그린조아", "갑자기 약속 취소하고 잠수타셔서" +
                " 시간만 낭비했네요"))
    }
}